...
\thusetup{
  ...
  %
  title  = {面向离子阱量子计算的电子学谐振腔及测控系统研究},
  title* = {Research on Electronic Resonantor and Measurement-Control System for Ion Trap Quantum Computing},
  ...
  %
  supervisor  = {路尧副研究员},
  supervisor* = {Assistant Researcher LU Yao},
  % 副指导教师
  %   一般无需启用该项，留空或者注释掉即可。
  %   如需启用限填写一名，且需要向学位办确认和备案，职称要求同指导教师。
  % associate-supervisor  = {张君华副研究员},
  % associate-supervisor* = {Assistant Researcher ZHANG Junhua},
  %
  % 日期
  %   使用 ISO 格式；默认为当前时间
