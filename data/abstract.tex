...
\begin{abstract}
  ...
  本文介绍了囚禁离子量子比特的准备和控制，面向离子量子计算系统展示了一种强实时性、分布式、易拓展的RTMQ量子测控系统架构以及与之相配套的指令集与时序控制结构；
  % 并给出若干重要片上外设拓展的设计及FPGA实现，如、、、高速通用数字PID及高速通用数字IIR滤波器等；
  使用有限元仿真及实验的方法对离子阱系统中一种重要的测控器件——螺线管谐振腔进行了仿真、实验、建模、设计等方面的优化研究，给出了设计特定频率下的高Q谐振腔的方法，同时也针对离子阱系统设计了更方便易用的谐振腔机械结构；
